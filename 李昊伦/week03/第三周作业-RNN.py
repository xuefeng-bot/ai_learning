"""
train_chinese_cls_rnn_multiclass.py
中文"你"字位置分类 —— RNN 多分类版本

任务：输入一个包含"你"字的五个字的文本，判断"你"在第几位（1-5）
模型：Embedding → RNN → 取最后隐藏状态 → Linear → Softmax → CrossEntropyLoss
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):
        return (
            ...
            torch.tensor(self.y[i], dtype=torch.long),  # 改为long类型
        )

# ─── 4. 模型定义 ────────────────────────────────────────────
class YouPositionRNN(nn.Module):
    """
    "你"字位置分类器（RNN + 多分类版）
    架构：Embedding → RNN → 取最后隐藏状态 → Linear → Softmax → CrossEntropyLoss
    """
    ...
    def forward(self, x):
        ...
        # 取最后一个时间步的隐藏状态
        last_hidden = e[:, -1, :]            # (B, hidden_dim)
        last_hidden = self.dropout(self.bn(last_hidden))
        out = self.fc(last_hidden)           # (B, num_classes) - 不加softmax，CrossEntropyLoss会处理
        return out
...
def train():
    ...
    model     = YouPositionRNN(vocab_size=len(vocab), num_classes=NUM_CLASSES)
