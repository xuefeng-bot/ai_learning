...
class TorchModel(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.linear = nn.Linear(input_size, output_size)  # 线性层，输出5维
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            # y是类别索引，不需要one-hot
            # 伪代码，交叉熵内部等价于：
            #probs = softmax(logits)  # 第1步：转成概率分布
            #loss = -log(probs[target])  # 第2步：计算交叉熵
            return self.loss(x, y)  # 预测值和真实值计算损失
        else:
            return x  # 输出原始logits，用argmax获取预测类别


# 生成一个样本, 样本的生成方法，代表了我们要学习的规律
# 随机生成一个5维向量，哪一维数字最大就属于第几类（0-4）
def build_sample():
    x = np.random.random(5)
    label = np.argmax(x)  # 最大值的索引作为类别标签
    return x, label
...
def predict(model_path, input_vec):
    ...
    with torch.no_grad():  # 不计算梯度
        result = model.forward(torch.FloatTensor(input_vec))  # 模型预测，返回logits
        pred_class = torch.argmax(result, dim=1)  # 获取预测类别
